    ...
    <div class="pt-8 pb-7 flex"
        ...
        <a href="/">
            <div x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                ...
                <div class="text-2xl font-bold text-brand-500 tracking-wider flex items-center gap-2">
                    <img src="/images/logo/jata-negeri-perak.png" alt="SiKEM Logo" class="h-10 w-auto" />
                    <span>SiKEM</span>
                </div>
                <span class="text-[11px] font-normal text-gray-400 dark:text-gray-500 block leading-tight ml-1 whitespace-nowrap overflow-hidden text-ellipsis">(Sistem Integrasi Kebajikan Mualaf)</span>
            </div>
            <div x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                class="text-2xl font-bold text-brand-500">
                <img src="/images/logo/jata-negeri-perak.png" alt="SiKEM Logo" class="h-10 w-auto mx-auto" />
            </div>
        </a>
    </div>
                    ...
                    <div>
                        ...
                        <ul class="flex flex-col gap-1">
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                @php
                                    $showItem = true;
                                    if ($item['path'] === '/pusat-pengurusan' && !auth()->user()->hasAnyRole(['super_admin', 'kudd', 'mubaligh', 'guru_apim'])) {
                                        $showItem = false;
                                    }
                                @endphp
                                @if($showItem)
                                <li>
                                    ...
                                    @else
                                        <!-- Simple Menu Item -->
                                        <a href="{{ $item['path'] }}" 
                                            @if($item['path'] === '/panduan') onclick="window.dispatchEvent(new CustomEvent('open-panduan')); return false;" @endif

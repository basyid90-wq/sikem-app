<div class="relative" x-data="{
    ...
    >
        ...
        <div class="flex items-center justify-between pb-3 mb-3 border-b border-gray-100 dark:border-gray-800">
            <h5 class="text-lg font-semibold text-gray-800 dark:text-white/90">Notifikasi ({{ $notifications->count() }})</h5>
        ...
        <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
            @forelse($notifications as $notif)
                @php
                    ...
                    $targetUrl = isset($notifData['url']) ? $notifData['url'] : '#';
                    $targetMsg = isset($notifData['message']) ? $notifData['message'] : 'Notifikasi Baru diterima.';
                @endphp
                <li wire:click="markAsRead('{{ $notif->id }}')">
                    <a class="flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5" href="{{ $targetUrl }}">
        ...
        @if($notifications->count() > 0)
            <!-- Mark All Button -->
            <a href="#" class="mt-3 flex justify-center rounded-lg border border-gray-300 bg-white p-3 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200" wire:click.prevent="markAllAsRead">
                Tanda Semua Sudah Dibaca
            </a>
        @endif
    </div>
    <!-- Dropdown End -->
